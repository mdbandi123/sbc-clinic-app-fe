...
export const getQueueOfPatientsNotCheckedIn = async () => {
  const response = await axios.get(`${BASE_URL}/api/queue/details/list/checkin/false`);
  return response.data;
}

export const updateCheckIn = async ({params,formState}) => {
  const response = await axios.patch(`${BASE_URL}/api/queue/update/checkin/${params}`, formState, {
    headers: {
