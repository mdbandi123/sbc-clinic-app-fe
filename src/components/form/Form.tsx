...
function Form({state, handleDispatch, formType, staffChoices = []}: FormProps){
  ...
  const handleDetailsChange:ChangeEventHandler<HTMLInputElement | HTMLTextAreaElement> = (e) => {
    ...
    if(formType === 'report'){
      return (<>
        <Grid2 container spacing={3}>
          ...
          <Grid2 size={4}>
            <SelectDropdown label={"Generated by"} onChange={handleStaffChange} value={state.staff} isRequired={true}>
                {
                  staffChoices.map((item, idx) => {
                    return (
                      <MenuItem key={idx} value={item.staffId}>{item.name}</MenuItem>
                    )
                  })
                }
            </SelectDropdown>
